...
# Create DataFrame
data = {
    "Name": ["Manoj", "Kumar", "Manu", "Arjun", "Arun", "Kiran"],
...
df["Average"] = df[["Python", "SQL", "ML"]].mean(axis=1) #axis=1 → across the rows,axis=0 → down the columns
...
print(df.groupby("Branch")["ML"].mean()) #Group students by Branch → select ML scores → calculate the average ML score for each Branch.
...
print("\n12. Student with lowest Attendance:")
print(df.loc[df["Attendance"].idxmin()]) #Find the index/row number where the minimum value occurs.
